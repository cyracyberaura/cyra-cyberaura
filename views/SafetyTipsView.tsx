...
const SafetyTipsView: React.FC<SafetyTipsViewProps> = ({ onBack }) => {
  ...
  return (
    <div className="animate-in slide-in-from-right duration-300 pb-12">
      ...
      <h1 className="text-2xl font-black mb-2 uppercase tracking-tight">Safety Advice</h1>
      <p className="text-sm text-slate-400 mb-8 font-medium">Live security recommendations generated for your device.</p>

      {loading ? (
        <div className="flex flex-col items-center justify-center py-24 gap-4">
          <div className="w-10 h-10 border-4 border-cyan-500/20 border-t-cyan-500 rounded-full animate-spin"></div>
          <p className="text-[10px] text-slate-500 font-black tracking-widest uppercase">Checking Threat Database...</p>
        </div>
      ) : (
        <div className="space-y-6">
          <div className="p-1 bg-gradient-to-r from-cyan-500 to-blue-600 rounded-3xl">
             <div className="bg-slate-900 rounded-[22px] p-6">
               <h2 className="text-xl font-black mb-1">{data?.title || 'System Recommendations'}</h2>
               <p className="text-[10px] text-slate-500 uppercase font-bold tracking-widest">Update: Just Now</p>
             </div>
          </div>

          <div className="space-y-4">
            <h3 className="text-xs font-black text-slate-500 uppercase tracking-widest px-2">Actionable Advice</h3>
            {data?.tips.map((tip, i) => (
              <div 
                key={i} 
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    {tip.urgent && (
                      <span className="text-[8px] font-black text-rose-500 uppercase tracking-widest block mb-1">High Priority</span>
                    )}
                    <p className="text-sm text-slate-200 leading-relaxed font-medium">{tip.text}</p>
                  </div>
                </div>
              </div>
            ))}
