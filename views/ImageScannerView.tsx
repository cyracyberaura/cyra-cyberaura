...
const ImageScannerView: React.FC<ImageScannerViewProps> = ({ onBack }) => {
  ...
  return (
    <div className="animate-in slide-in-from-right duration-300 pb-12">
      ...
      <h1 className="text-2xl font-black mb-2 uppercase tracking-tight">Picture Scanner</h1>
      <p className="text-sm text-slate-400 mb-8 leading-relaxed font-medium">Scan your photos to see if they are safe and get details about what is in them.</p>
      ...
      {!preview ? (
        <div className="border-2 border-dashed border-slate-700 rounded-3xl p-16 flex flex-col items-center justify-center text-center gap-4 bg-slate-800/20 relative group hover:border-cyan-500/50 transition-all">
          ...
          <div className="space-y-1">
            <p className="text-sm font-bold text-slate-200 uppercase">Tap to Add Photo</p>
            <p className="text-[10px] text-slate-500 uppercase tracking-widest font-black">Screenshots or Downloads</p>
          </div>
        ...
        <div className="space-y-6">
          <div className="relative rounded-3xl overflow-hidden border-2 border-slate-700/50 shadow-2xl bg-slate-950">
            ...
            {loading && (
              <div className="absolute inset-0 bg-slate-900/80 backdrop-blur-sm flex flex-col items-center justify-center gap-4">
                ...
                <p className="text-cyan-400 font-black text-xs uppercase tracking-[0.3em] animate-pulse">Scanning...</p>
              </div>
            )}
          </div>
          ...
          {result && (
            <div className="space-y-4 animate-in slide-in-from-bottom-6 duration-500">
              <h3 className="text-xs font-black text-cyan-400 uppercase tracking-[0.2em] px-1">What we found</h3>
              
              <div className="p-5 rounded-2xl bg-slate-800/50 border border-slate-700/50 grid grid-cols-2 gap-6">
                <div>
                  <span className="text-[10px] font-black text-slate-500 uppercase block mb-1">Is it safe?</span>
                ...
                <div>
                  <span className="text-[10px] font-black text-slate-500 uppercase block mb-1">Source</span>
                  <span className="text-xs font-black text-white truncate block uppercase">{result.imageOrigin || "Unknown"}</span>
                </div>
              </div>

              <div className="p-5 rounded-2xl bg-slate-800/50 border border-slate-700/50 space-y-4">
                <div>
                  <span className="text-[10px] font-black text-slate-500 uppercase block mb-1">Picture Details</span>
                  <p className="text-xs text-slate-300 leading-relaxed font-bold uppercase">{result.explanation}</p>
                </div>
                <div className="pt-3 border-t border-slate-700/50">
                  <span className="text-[10px] font-black text-slate-500 uppercase block mb-1">Extra Details</span>
                  <p className="text-[11px] text-cyan-400/80 font-bold uppercase tracking-tight">{result.technicalDetails}</p>
                </div>
              </div>

              <div className="p-5 rounded-2xl bg-slate-950/50 border border-slate-700/50 shadow-inner">
                <span className="text-[10px] font-black text-slate-500 uppercase block mb-3 tracking-widest">Points to follow</span>
                <ul className="space-y-3">
                  {result.recommendations.map((r, i) => (
                    <li key={i} className="text-xs text-slate-400 flex gap-3 items-start">
                      ...
                      <span className="font-bold uppercase tracking-tight">{r}</span>
                    </li>
                  ))}
                </ul>
              </div>
